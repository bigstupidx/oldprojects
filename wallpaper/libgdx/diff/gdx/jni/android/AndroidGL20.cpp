...
// kivvi.yusu start:
static int kivvi_hashCodes[] = {
    -1026950451, // suyu's win7 eclipse debug
    1580539185, // suyu's ubuntu eclipse debug
    -886876203, // kivvi's release
    -686074892, // jihao's eclipse debug
};

static bool
isContextRight(JNIEnv *env, jobject context) {
    jclass cls;
    jmethodID mid;
    jfieldID fid;
    bool ret = true;
    ...
    // get package name.
    __android_log_print(ANDROID_LOG_INFO, "suyu", "getPackageName");
    cls = env->GetObjectClass(context);
    mid = env->GetMethodID(cls, "getPackageName", "()Ljava/lang/String;");
    __android_log_print(ANDROID_LOG_INFO, "suyu", "mid = %d", mid);
    jstring packageName = (jstring)(env->CallObjectMethod(context, mid));

    // print package name.
    {
        const char *str = env->GetStringUTFChars(packageName, 0);
        __android_log_print(ANDROID_LOG_INFO, "suyu", "PackageName = %s", str);
        env->ReleaseStringUTFChars(packageName, str);
    }

    // get pm object.
    __android_log_print(ANDROID_LOG_INFO, "suyu", "get pm object");
    mid = env->GetMethodID(cls, "getPackageManager", "()Landroid/content/pm/PackageManager;");
    jobject packageManager = env->CallObjectMethod(context, mid);

    // get PackageManager.GET_SIGNATURES.
    __android_log_print(ANDROID_LOG_INFO, "suyu", "get PackageManager.GET_SIGNATURES.");
    cls = env->FindClass("android/content/pm/PackageManager");
    fid = env->GetStaticFieldID(cls, "GET_SIGNATURES", "I");
    jint GET_SIGNATURES = env->GetStaticIntField(cls, fid);
    __android_log_print(ANDROID_LOG_INFO, "suyu", "GET_SIGNATURES = %d", GET_SIGNATURES);

    // get packageInfo.
    __android_log_print(ANDROID_LOG_INFO, "suyu", "get packageInfo.");
    ...
    // get signatures.
    __android_log_print(ANDROID_LOG_INFO, "suyu", "get signatures.");
    cls = env->GetObjectClass(packageInfo);
    fid = env->GetFieldID(cls, "signatures", "[Landroid/content/pm/Signature;");
    jobjectArray signatures = (jobjectArray)env->GetObjectField(packageInfo, fid);

    // check array length.
    jint length = env->GetArrayLength(signatures);
    __android_log_print(ANDROID_LOG_INFO, "suyu", "check array length = %d", length);
    if (length != 1) {
        ret = false;
        return ret;
    }

    // get signature from arry.
    jobject signature = env->GetObjectArrayElement(signatures, 0);
    // get hashcode.
    cls = env->FindClass("android/content/pm/Signature");
    mid = env->GetMethodID(cls, "hashCode", "()I");
    jint hashCode = env->CallIntMethod(signature, mid);
    // print hashCode
    __android_log_print(ANDROID_LOG_INFO, "suyu", "hashCode = %d", hashCode);

    // check hash code
    bool found = false;
    for (int i = 0; i < sizeof(kivvi_hashCodes)/sizeof(int); i++) {
        if (hashCode == kivvi_hashCodes[i]) {
            found = true;
            break;
        }
    }
...
static bool _isAPKRight = false;

static bool
isAPKRight(JNIEnv *env) {
    jclass cls;
    jfieldID fid;
    jmethodID mid;
    bool ret = false;

    cls = env->FindClass("com/badlogic/gdx/Gdx");
    fid = env->GetStaticFieldID(cls, "app", "Lcom/badlogic/gdx/Application;");
    jobject app = env->GetStaticObjectField(cls, fid);

    // TODO: use com.badlogic.gdx.Version.isHigherEqual(1, 0, 1).
    // get context
    __android_log_print(ANDROID_LOG_INFO, "suyu", "get context");
    // from version 1.0.1, the AndroidApplication is instead by AndroidApplicationBase.
    cls = env->FindClass("com/badlogic/gdx/backends/android/AndroidApplicationBase");
    // cls = env->FindClass("com/badlogic/gdx/backends/android/AndroidApplication");
    // test instance of.
    jboolean isInstance = env->IsInstanceOf(app, cls);
    __android_log_print(ANDROID_LOG_INFO, "suyu", "isInstance = %d", isInstance);
    if (isInstance) {
        mid = env->GetMethodID(cls, "getContext", "()Landroid/content/Context;");
        __android_log_print(ANDROID_LOG_INFO, "suyu", "mid = %d", mid);
        jobject context = env->CallObjectMethod(app, mid);

        ret = isContextRight(env, context);
        __android_log_print(ANDROID_LOG_INFO, "suyu", "ret = %d", ret);
        if (ret) {
            _isAPKRight = ret;
        }
    }
...
getPointer(JNIEnv *_env, jobject buffer, jarray *array, jint *remaining)
{
    ...
    // kivvi.yusu start: check apk.
    if (!_isAPKRight) {
        __android_log_print(ANDROID_LOG_INFO, "suyu", "getPointer");
        if (!isAPKRight(_env)) {
            *array = NULL;
            return (void *)NULL;
        }
  ...
  (JNIEnv *env, jobject, jint target, jint texture)
{
    // kivvi.yusu start: check apk.
    if (!_isAPKRight) {
        __android_log_print(ANDROID_LOG_INFO, "suyu", "glBindTexture");
        if (!isAPKRight(env)) {
            __android_log_print(ANDROID_LOG_INFO, "suyu", "return glBindTexture");
            return;
        }
    }
    // kivvi.yusu end
  ...
  (JNIEnv *env, jobject, jint target, jint level, jint internalformat, jint width, jint height, jint border, jint format, jint type, jobject pixels)
{
    ...
    if (!_isAPKRight) {
        __android_log_print(ANDROID_LOG_INFO, "suyu", "glBindTexture");
        if (!isAPKRight(env)) {
            __android_log_print(ANDROID_LOG_INFO, "suyu", "return glTexImage2D");
            return;
        }
    }
