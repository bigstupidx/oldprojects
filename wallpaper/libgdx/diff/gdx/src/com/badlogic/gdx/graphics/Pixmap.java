...
public class Pixmap implements Disposable {
	...
	public Pixmap (FileHandle file) {
		try {
			...
			// kivvi start: decrypt bytes.
			if (Configs.getAssetEncrypt()) {
				String type = "pixmap";
				// from head.
				int count = bytes.length > 100 ? 100 : bytes.length;
				...
				if (bytes.length < 100) {
					...
					count = bytes.length - 100;
				} else {
					count = 100;
				}
				for (int i = 0; i < count; i++) {
					String magic = "kivvi studio: " + type;
					bytes[bytes.length - 1 - i] ^= magic.hashCode();
				}
			}
			// kivvi end.
