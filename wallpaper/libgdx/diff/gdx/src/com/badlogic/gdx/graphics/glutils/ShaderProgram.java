...
public class ShaderProgram implements Disposable {
	...
	// Kivvi start: encrypt glsl.
	public static String encryptReadString (FileHandle f) {
		if (com.badlogic.gdx.Configs.getAssetEncrypt()) {
			byte[] bytes = f.readBytes();
			String magic = "kivvi studio: shader";
			for (int i = 0; i < bytes.length; i++) {
				bytes[i] ^= magic.hashCode();
			}
	...
	public ShaderProgram (FileHandle vertexShader, FileHandle fragmentShader) {
		// Kivvi start: encrypt glsl.
		this(encryptReadString(vertexShader), encryptReadString(fragmentShader));
		// Kivvi end.
